...
export function BettingForm({ currentPrice, game, balance, onBet, isBetting = false, userBets = [], allPrices = {}, underMaintenance = false, onDurationChange }: BettingFormProps & { onDurationChange?: (d: 180 | 300) => void }) {
  ...
  // Track round changes for ALL games simultaneously
  useEffect(() => {
    const checkAllGames = () => {
      if (document.hidden) return; // 모바일: 탭 비활성 시 건너뜀
      const kstTime = getKSTDate();
    ...
    // 초기 렌더 완료 후 1.5초 뒤 시작 — 첫 마운트 CPU 스파이크 방지
  ...
  return (
    <div className="relative flex flex-col lg:h-full bg-card w-full">
      ...
      <div className="p-3 lg:p-4 space-y-3 lg:space-y-3 lg:flex-1 lg:overflow-y-auto lg:flex lg:flex-col">
        ...
        <div className="hidden sm:flex items-center justify-between px-2 py-1.5 rounded bg-muted/30 border border-border">
          ...
          >
            {Math.floor(availableBalance).toLocaleString()}원
          </span>
        </div>
        ...
        {currentRoundPendingBet ? (
          ...
          <div className="flex flex-col items-center justify-center gap-3 py-4 px-3 rounded-lg bg-primary/10 border border-primary/20">
            ...
            <div className={cn(
              ...
              {currentRoundPendingBet.direction === 'long' ? 'Long' : 'Short'} 주문 완료
            </div>
          ...
          <>
            <div className="space-y-2">
              ...
              <div className="relative">
                <Input 
                  ...
                  onPaste={handlePaste}
                  onCopy={handleCopy}
                  className="font-mono text-sm text-right pr-8 h-10 min-h-[40px] bg-input border-border focus-visible:ring-primary [appearance:textfield] touch-manipulation"
                  data-testid="input-bet-amount"
                  placeholder="금액 입력"
                />
                <span className="absolute right-3 top-1/2 -translate-y-1/2 text-sm text-muted-foreground pointer-events-none">원</span>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-2 pt-1 lg:pt-2">
              <Button 
                ...
                className="h-10 min-h-[40px] text-xs font-bold text-white flex items-center justify-center gap-1 w-full bg-gray-500 hover:bg-gray-400 active:scale-[0.98] touch-manipulation"
              ...
              <div className="flex gap-2">
                <Button 
                  ...
                  className={cn(
                    "h-12 lg:h-14 text-base font-bold text-white flex items-center justify-center gap-1.5 flex-1 active:scale-[0.98] touch-manipulation",
                    isBettingLocked 
        ...
        <div className="border-t border-border pt-3 lg:flex-1 lg:flex lg:flex-col lg:min-h-0">
          ...
          {gameResults.length === 0 ? (
            ...
            <ScrollArea className="lg:flex-1 lg:h-0 lg:min-h-[120px] max-h-[220px] lg:max-h-none">
