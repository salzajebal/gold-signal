...
export async function registerRoutes(
  ...
  app.post("/api/bets", requireAuth, async (req, res) => {
    try {
      ...
      const VALID_SYMBOLS = ['GOLD', 'GBP', 'BTC', 'SILVER'];
      if (!VALID_SYMBOLS.includes(symbol)) {
        return res.status(400).json({ error: "Invalid symbol. Only GOLD, GBP, BTC, SILVER are available." });
      }
  ...
  app.get("/api/affiliate/global-forced", requireAffiliate, async (req, res) => {
    try {
      const symbols = ['GOLD', 'GBP', 'BTC', 'SILVER'];
  ...
  // Yahoo Finance symbols for reverse lookup
  ...
  const forexPrices: { [key: string]: { price: number; change: number; changePercent: number; high: number; low: number; volume: number; updatedAt: number; openPrice: number } } = {
    ...
    GBP: { price: 0, change: 0, changePercent: 0, high: 0, low: 0, volume: 0, updatedAt: 0, openPrice: 0 },
    BTC: { price: 0, change: 0, changePercent: 0, high: 0, low: 0, volume: 0, updatedAt: 0, openPrice: 0 },
    SILVER: { price: 0, change: 0, changePercent: 0, high: 0, low: 0, volume: 0, updatedAt: 0, openPrice: 0 },
  };
  ...
  async function loadCandlesFromDB() {
    ...
    const totalCandles = symbols.reduce((sum, s) => sum + durations.reduce((s2, d) => s2 + (candleStore[s]?.[d]?.length ?? 0), 0), 0);
  ...
  function scheduleCandleDbSave(symbol: string, dur: number, candle: CandleData) {
    ...
    if (!dbSaveTimer) {
      dbSaveTimer = setTimeout(async () => {
        ...
        for (const sym of ['GOLD', 'GBP', 'BTC', 'SILVER']) {
          for (const d of [180, 300]) {
            if (candleStore[sym]?.[d] && candleStore[sym][d].length > MAX_CANDLES + 50) {
  ...
  async function fetchYahooPrices(): Promise<void> {
    try {
      ...
      if (updated > 0) {
        ...
        isConnected = true;
        console.log(`💹 [Yahoo] 실시간 시세: GOLD=${forexPrices.GOLD.price.toFixed(2)}, GBP=${forexPrices.GBP.price.toFixed(4)}, BTC=${forexPrices.BTC.price.toFixed(2)}, SILVER=${forexPrices.SILVER.price.toFixed(3)}`);
  ...
  function applyMicroFluctuation() {
    const now = Date.now();
    const volMap: Record<string, number> = { GOLD: 0.00003, GBP: 0.000025, BTC: 0.00006, SILVER: 0.00004 };
    for (const symbol of ['GOLD', 'GBP', 'BTC', 'SILVER']) {
      const prev = forexPrices[symbol];
      if (prev.price <= 0) continue;
      const vol = volMap[symbol] || 0.00004;
  ...
  // 1초마다 미세 변동
  setInterval(() => {
    if (Object.values(forexPrices).some(p => p.price > 0)) applyMicroFluctuation();
  }, 1000);
  ...
  function startSimulation() {
    ...
    simulationTimer = setInterval(() => {
      ...
      const now = Date.now();
      const simVolMap: Record<string, number> = { GOLD: 0.00005, GBP: 0.00005, BTC: 0.0001, SILVER: 0.00006 };
      for (const symbol of ['GOLD', 'GBP', 'BTC', 'SILVER']) {
        const prev = forexPrices[symbol];
        const currentPrice = prev.price > 0 ? prev.price : DEFAULT_PRICES[symbol];
        const anchor = simulationAnchorPrices[symbol] || currentPrice;
        const vol = simVolMap[symbol] || 0.00005;
  ...
  app.get("/api/market/prices", (req, res) => {
    ...
    for (const forexSymbol of ['GOLD', 'GBP', 'BTC', 'SILVER']) {
  ...
  app.get("/api/market/price/:symbol", (req, res) => {
    ...
    const VALID_FOREX = ['GOLD', 'GBP', 'BTC', 'SILVER'];
